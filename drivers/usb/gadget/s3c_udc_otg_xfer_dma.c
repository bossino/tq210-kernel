...
//#define GINTMSK_INIT	(INT_OUT_EP|INT_IN_EP|INT_RESUME|INT_ENUMDONE|INT_RESET|INT_SUSPEND|INT_CONN_ID_STS_CNG)
//Apollo -
...
extern	struct workqueue_struct *my_usb_diable_wq;
...
//#endif
	...
	if (intr_status & INT_RESET) {
		...
		if ((usb_status & 0xc0000) == (0x3 << 18)) {
			...
			//fighter++
			#if defined(CONFIG_S5PV210_S9) || defined(CONFIG_S5PV210_A9)
			queue_work(my_usb_diable_wq, &my_work);
			#endif
			//fighter--
...
////Apollo +
//	if (intr_status & INT_CONN_ID_STS_CNG)
//	{
////		usb_status = readl(S3C_UDC_OTG_DSTS);
////		DEBUG_ISR("\tSuspend interrupt :(DSTS):0x%x\n", usb_status);
//		writel(INT_CONN_ID_STS_CNG, S3C_UDC_OTG_GINTSTS);
//		
//		usb_status = readl(S3C_UDC_OTG_GOTGCTL);
//	
////	  //reset
...
////		s3c_udc_pre_setup();
//		
//    if(usb_status & (0x1<<16))
//    {
//printk("otg change to device\n");
////      reconfig_usbd();
////      pOtgPhyReg->OPHYCLK &= ~(0x1<<6);
////      usb_status = readl(S3C_UDC_OTG_GUSBCFG);
////  		writel(usb_status & ~(0x3<<29) | (0x1<<30), S3C_UDC_OTG_GUSBCFG);
////  		mdelay(30);
...
////      		writel((PTX_FIFO_SIZE) << 16 | (NPTX_FIFO_START_ADDR + NPTX_FIFO_SIZE + PTX_FIFO_SIZE*(i-1)) << 0, S3C_UDC_OTG_DIEPTXF(i));
////      #endif
//    }
//    else
//    {
//printk("otg change to host\n");
////      pOtgPhyReg->OPHYCLK |= (0x1<<6);
////	    writel((NPTX_FIFO_SIZE) << 16 | (RX_FIFO_SIZE) << 0, S3C_UDC_OTG_GNPTXFSIZ);
////  		writel((NPTX_FIFO_SIZE) << 16 | (RX_FIFO_SIZE + NPTX_FIFO_SIZE), S3C_UDC_OTG_HPTXFSIZ);
////      usb_status = readl(S3C_UDC_OTG_GUSBCFG);
////  		writel(usb_status & ~(0x3<<29) | (0x1<<29), S3C_UDC_OTG_GUSBCFG);
//     }
//	}
////Apollo -
	spin_unlock_irqrestore(&dev->lock, flags);
