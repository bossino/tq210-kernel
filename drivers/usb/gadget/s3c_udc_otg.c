...
struct workqueue_struct *my_usb_diable_wq;
...
EXPORT_SYMBOL(s3c_udc_soft_connect);
...
void s3c_udc_soft_disconnect(void)
{
	...
	struct s3c_udc *dev = the_controller;
	unsigned long flags;
	//printk("s3c_udc_soft_disconnect++++++++++++++++++++++++++++++++++++\n");
	DEBUG("[%s]\n", __func__);
...
EXPORT_SYMBOL(s3c_udc_soft_disconnect);
...
//	}
	...
	create_proc_files();
		...
		//fighter++
#if defined(CONFIG_S5PV210_S9) || defined(CONFIG_S5PV210_A9)
	my_pdev = kzalloc(sizeof(struct platform_device), GFP_KERNEL);
	if (my_pdev == NULL)
		return -ENOMEM;
	memcpy(my_pdev,pdev,sizeof(struct platform_device));
	
	my_usb_diable_wq = create_workqueue("my_usb_diable_wq");
	if (!my_usb_diable_wq) {
...
//	//fighter++
//	/*
//	 S9 休眠后唤醒时若USB已断开连接就再主动断开一次
//	*/
//	#if defined(CONFIG_S5PV210_S9)
//	printk("usb resume++++++++++++++++++++++++++\n");
//	tmp = readl(S3C_UDC_OTG_GOTGCTL);
