...
//Apollo +
static struct timer_list hub_timer;
extern void usbhub_power(int onoff);

void hub_power(unsigned long param)
{
//printk("hub_power\n");
...
dead:
	if (unlikely ((status & STS_FATAL) != 0))
	{
	  printk("echi fatal error\n");
    hub_timer.expires = jiffies + HZ/2;
    add_timer(&hub_timer);
	}
//Apollo -
...
#endif
//Apollo +
  init_timer(&hub_timer);
  hub_timer.data = 0;
  hub_timer.function = hub_power;
//Apollo -
	return retval;
