...
int s5p_hpd_set_hdmiint(void)
{
	...
	set_irq_type(GPIO_HDMI_HPD_IRQ, IRQ_TYPE_NONE);
	...
	s3c_gpio_cfgpin(GPIO_HDMI_HPD, S3C_GPIO_SFN(0x4));
...
printk("%s\n", __func__);
	...
	s3c_gpio_cfgpin(GPIO_HDMI_HPD, GPIO_HDMI_HPD_CFG);
	s3c_gpio_setpull(GPIO_HDMI_HPD, S3C_GPIO_PULL_DOWN);
	writel(readl(S5PV210_GPH1DRV)|0x3<<10, S5PV210_GPH1DRV);
	...
	set_irq_type(GPIO_HDMI_HPD_IRQ, IRQ_TYPE_EDGE_BOTH);

	if (request_irq(GPIO_HDMI_HPD_IRQ, s5p_hpd_irq_handler, IRQF_DISABLED,
