...
drv_iwevt_handler(struct nlmsghdr *h, int left, int *evt_conn)
{
    ...
    *evt_conn = FALSE;
    while ((unsigned int) left >= sizeof(*h)) {
        ...
        if ((unsigned int) plen < sizeof(*ifi)) {
            break;
        }
        ifi = NLMSG_DATA(h);
...
run_app(void)
{
    ...
    while (!terminate_flag) {
        ...
        left = recv(nl_sk, msg, MAX_MSG_LENGTH, MSG_DONTWAIT);
