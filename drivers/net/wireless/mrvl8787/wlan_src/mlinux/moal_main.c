...
void wifi_set_power(int on)
{
  printk("%s %d\n", __func__, on);
	wlan_setup_power(0, on);
  if((on==1) && (get_wlan_power_state() != 0x03))
    wlan_carddetect_en(1);
  if((on==0) && (get_wlan_power_state() == 0))
    wlan_carddetect_en(0);
}
//proc/wifistate/state
#ifndef CONFIG_MRVL8787_SUSPEND_KEEPPOWER
#include <linux/platform_device.h>

unsigned long suspend = 0;
struct proc_dir_entry *proc_state = NULL;
static int get_state(char *page, char **s, off_t off, int cnt, int *eof, void *data)
{
  int ret;
  ret = sprintf(page, "%ld\n", suspend);
//printk("get wifi suspend = %ld\n", suspend);
  if(suspend == 2)
    suspend = 0;
	return ret;
}

static int set_state(struct file *f, const char *buf, unsigned long cnt, void *data)
{
  int rc;
  char databuf[100];
  ...
  rc = strict_strtoul(databuf, 0, &suspend);
printk("set wifi suspend = %ld\n", suspend);
...
static struct platform_driver mrvl8787_device = {
	.probe          = wifi_probe,
	.remove         = wifi_remove,
...
#endif /* CONFIG_MRVL8787_SUSPEND_KEEPPOWER */
//Apollo -
...
//    if ((bss_type == MLAN_BSS_TYPE_STA) && (dev_alloc_name(dev, "mlan%d") < 0)) {
    if ((bss_type == MLAN_BSS_TYPE_STA) && (dev_alloc_name(dev, "eth0") < 0)) {
//Apollo -
...
//Apollo +
...
#ifndef CONFIG_MRVL8787_SUSPEND_KEEPPOWER
    {
      ...
      proc_state = proc_mkdir("wifistate", NULL);
      if (proc_state)
      {
        printk("create wifistate file\n");
        r = create_proc_entry("state", 0777, proc_state);
        if (r)
        {
          r->read_proc = get_state;
          r->write_proc = set_state;
        }
      }
    }
  platform_driver_register(&mrvl8787_device);
#endif
//Apollo -
    LEAVE();
    return ret;
...
//Apollo +
    wifi_set_power(0);
#ifndef CONFIG_MRVL8787_SUSPEND_KEEPPOWER
  remove_proc_entry("state", proc_state);
  remove_proc_entry("wifistate", NULL);

  platform_driver_unregister(&mrvl8787_device);
#endif
//Apollo -
    LEAVE();
}
