...
#define dm9620_print(__dev, format, args...) devdbg(__dev, format, ##args)
#define dm9620_err(__dev, format, args...) deverr(__dev, format, ##args)
#endif

static int dm_read(struct usbnet *dev, u8 reg, u16 length, void *data)
{
//	netdev_dbg(dev->net, "dm_read() reg=0x%02x length=%d", reg, length);
...
{
//	netdev_dbg(dev->net, "dm_write() reg=0x%02x, length=%d", reg, length);
...
{
//	netdev_dbg(dev->net, "dm_write_reg() reg=0x%02x, value=0x%02x", reg, value);
...
{
//	netdev_dbg(dev->net, "dm_write_async() reg=0x%02x length=%d", reg, length);
...
//	netdev_dbg(dev->net, "EEPROM: magic value, magic = 0x%x offset =0x%x data = 0x%x ",eeprom->magic, eeprom->offset,*data);
...
//		netdev_dbg(dev->net, "Only internal phy supported");
		dm9620_print(dev, "Only internal phy supported");
		return 0;
	}
...
//JJ3
	...
	udelay(20);
	//Stone add Enable Flow Control and TX Pause Packet Enable
	dm_write_reg(dev, DM_FLOW_CTRL, 0x21);
	...
	if (!priv) {
//		netdev_err(dev->net,"Failed to allocate memory for dm96xx_priv");
