...
//printk("%s: starting CMD%u arg %08x flags %08x\n", mmc_hostname(host), mrq->cmd->opcode, mrq->cmd->arg, mrq->cmd->flags);
  if((strcmp(mmc_hostname(host), "mmc0")==0) && (resume_flag == 1))
  {
    unsigned long tempp;
    tempp = jiffies - mmc_last_resume;
//printk("mmc resume sleep read %lu %lu\n", mmc_last_resume, jiffies);
    if((tempp/HZ) < 3)
    {
      msleep(3000-tempp/HZ*1000);
      printk("mmc resume true sleep \n");
    }
    resume_flag = 0;
  }
