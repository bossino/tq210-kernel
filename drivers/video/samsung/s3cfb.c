...
//Apollo +
#define BOOTLOADER_LOGO_ADDRESS (0x3F000000)
//Apollo -
...
static int s3cfb_draw_logo_bootloader(struct fb_info *fb, void *src_base)
{
	struct fb_fix_screeninfo *fix = &fb->fix;
	struct fb_var_screeninfo *var = &fb->var;
  void *vsrc_base;
  
  vsrc_base = ioremap(src_base,fix->line_length * var->yres);

//  printk("aaaaaa: 0x%x 0x%x 0x%x\n\n",fb->screen_base, vsrc_base, fix->line_length * var->yres);
	memcpy(fb->screen_base, vsrc_base, fix->line_length * var->yres);
...
//Apollo +
      	s3cfb_draw_logo_bootloader(ctrl->fb[j], (void *)BOOTLOADER_LOGO_ADDRESS);
//Apollo -
...
//fighter++
	...
	ret = regulator_disable(fbdev->regulator);
	if (ret < 0) {
		dev_err(fbdev->dev, "failed to disable regulator\n");
		ret = -EINVAL;
		goto err_regulator;
	}
